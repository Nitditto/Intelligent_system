...
### 2.1 How this report meets the assignment's mandatory requirements

The assignment states, as a mandatory instruction: *"Students must explain the
representation of their data before training the model. Showing only the model architecture
is not sufficient."* It also states: *"Representation is part of the ML solution. Students
receive marks not only for obtaining good accuracy, but also for correctly explaining how
raw data becomes a computational representation."* This report is organised around those two
instructions - every application is taken from a raw record to a numerical feature matrix,
and the matrix shape and every dimension are stated, before any model is trained. The table
below maps each mandatory instruction to where it is answered.

| Mandatory instruction (from the assignment) | Where this report answers it |
|---|---|
| Explain the representation before the model; architecture alone is not enough | Sections 3 and 3.1, and every "Data representation" subsection (4.5, 5.4, 6.3), which show `raw record -> feature vector -> matrix shape` with one worked example per application, before any model is trained |
| Demonstrate how raw data becomes a numerical representation a model can process | The Section 2 diagram; the `raw -> vector -> matrix -> request` blocks in 4.5 / 5.4 / 6.3; Figures 4.1, 5.1 and 6.1 |
| Explain WHY each cleaning operation is performed | The "Why" column in every data-cleaning table (4.4, 5.3, 6.4) |
| Explain why test data must not influence model training or preprocessing fitting | Sections 4.7 and 6.6, and the "Data-leakage rule" in Section 8: every fitted object is fitted on `X_train` only and the deployed service never calls `.fit()` |
| For every major visualisation, give Observation / Interpretation / ML implication | The EDA analysis bullets in 4.6, 5.5 and 6.5 |
| Explain what each metric means in the application | The confusion-matrix readings in 4.8 and 6.8; the "what each metric means here" paragraph in 5.6 |
| Explain every dimension in the reported shapes | Section 3.1 ("Every dimension"), including the per-application breakdown of `d` |
| Consider data leakage | Section 6.6 (three concrete forms for Application 3) and Section 10 (five forms across all three applications) |
...
### 4.3 Data understanding and quality
...
### 4.8 Evaluation and model comparison
...
### 4.10 Persistence and deployment
...
### 5.6 Regression models and evaluation
...
### 5.7 Model selection, persistence and deployment
...
### 6.8 Evaluation and error analysis
...
### 6.10 Persistence and deployment
...
## Appendix D - Web Application Report
...
## Appendix E - Mobile Application Report

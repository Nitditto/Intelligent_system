...
\begin{tabular}{ll}
Student name: & Nguyễn Văn Trường \\[0.5cm]
...
Two results are worth highlighting before the detailed, per-application
sections below. The first is that every application ends up with exactly the
same parameter count across all three implementations, a fact each notebook
does not merely assume but actively checks
(\code{assert keras\_param\_count == scratch\_param\_count}, and an identical
check against PyTorch); this is the single most important sanity check in
the whole assignment, since it confirms the three implementations are
genuinely one architecture rather than three unrelated architectures that
happen to share a name. The second is that no implementation dominates
across applications: PyTorch wins on diabetes, scratch wins on Fashion-MNIST,
and Keras wins on CIFAR-10, with the margin between winner and runners-up
never exceeding a few accuracy points in any of the three cases. Rather than
undermining Lecture 04's claim, this shifting ranking supports it, because a
single implementation winning every single time would point to a real
difference in modeling power, whereas a ranking that reshuffles from
application to application is exactly what three equally capable
implementations of one architecture should produce, with the outcome decided
by small, framework-level details rather than by any implementation being
inherently the stronger learner. Training time is the far cleaner signal of
the two: Keras finishes fastest in every application, and the from-scratch
NumPy code falls dramatically behind the other two specifically once real
convolutions are involved, a concrete cost of full visibility that stays
hidden on the diabetes MLP, whose network is small enough that no
implementation's overhead has room to dominate.
...
The whole assignment is framed by one question Lecture 04 opens with: across
three implementations of the same CNN, from-scratch NumPy, TensorFlow/Keras,
and PyTorch, \textit{what actually stays the same, and what actually
changes?} The lecture answers this directly, the conceptual architecture does
not change at all between the three; only the level of abstraction does. In
scratch, a \code{Conv2d} is a concrete function carrying out an
\code{im2col} matrix multiply that can be read and traced line by line. In
Keras, the same operation collapses to a single line, \code{Conv2D(...)},
with the underlying multiply-and-accumulate work hidden inside a compiled
library routine. PyTorch's \code{nn.Conv2d(...)} lands in between the two:
the layer definition is as compact as Keras's, but the training loop invoking
it and the backward pass updating it remain explicit in the student's own
code instead of being folded into one \code{fit()} call.
...
On accuracy and F1, PyTorch finishes slightly in front (0.782, 0.480), with
Keras not far behind (0.753, 0.476) and scratch a similar small step further
back (0.731, 0.469). All three sit within roughly five points of one another
on both metrics, which is what should happen when the same architecture is
trained on the same data with the same hyperparameters three separate times:
a gap this size is easily attributable to framework-level details, default
numerical precision, exactly how each framework shuffles its batches, minor
differences in how the balanced weighting gets applied, rather than to one
implementation actually learning a meaningfully better model.
...
(CNN), and for each one implemented a single architecture three separate
times: from scratch in NumPy, with every forward pass, loss, backward pass,
and parameter update derived and coded by hand; in TensorFlow/Keras; and in
PyTorch. Across all three implementations, every application's parameter
count matched exactly and every application's accuracy spread stayed within
a few points, which together confirm Lecture 04's central claim that the
conceptual architecture stays fixed across abstraction levels and only its
expression changes. Training time, in contrast, genuinely diverged, and by a
large margin once real convolutions entered the picture: a from-scratch
NumPy implementation, no matter how correct, simply cannot match a mature
framework's operator-level optimization for something like convolution, a
real cost of full visibility that a small MLP's forward pass never exposes
but a CNN's does, and one that grows further with input size and channel
count rather than with parameter count. Keeping each implementation's
training loop and loss computation fully visible instead of relying on a
packaged \code{fit()} call is also what surfaced two concrete findings a
framework would otherwise have absorbed without a trace: PyTorch's
\code{pos\_weight} class-weighting mechanism scales the training loss
differently from scratch's and Keras's symmetric weighting despite sharing
the same balanced-weighting goal, and a redundant division bug in a
from-scratch backward pass that only a numerical gradient check, comparing
hand-derived gradients against a finite-difference approximation, was able to
catch. Both details are visible only to someone who wrote every gradient by
hand, which is exactly the payoff building a CNN from scratch is meant to
deliver.
